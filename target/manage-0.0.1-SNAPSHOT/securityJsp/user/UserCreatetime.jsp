...
function search(flag){
	   ...
	   $.post( sy.contextPath + '/mdmy/userController/findCountByDay',{'queryBeginRegisterTimeStr':begin,'queryEndRegisterTimeStr':end,'type':type}, function(result) {
			if(result!=null&&result.length>0){
				var data=new Array();
				var time=new Array();
					var headHtml="<table class=\"table\" style=\"font-size:12px;\"><tr><td>日期</td>";
					var bodyHtml="<tr><td>日注册人数</td>";
					var tableHtml="";
					for(var i=0;i<result.length;i++){
						...
						if(i!=0&&i%15==0){
							...
							bodyHtml="<tr><td>日注册人数</td><td>"+result[i].count+"</td>";
						}else{
 ...
 */
    ...
    function DrawEChart(ec) { 
        ...
        //图例配置 
        legend: { 
            data: ['注册人数'], //这里需要与series内的每一组数据的name值保持一致 
            y:"bottom" 
        }, 
        //工具箱配置 
         toolbox: { 
...
<br/>
   按类型：<select id="selected"  style="width: 90px;" name="type">
							<option value="1">日期</option>
							<option value="2">小时</option>
					</select>
   开始日期：<input type="text" name="queryBeginRegisterTimeStr" class="Wdate" id="begin" autocomplete="off" readonly="readonly"
 onclick="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'end\')||\'%y-%M-%d\'}'})" />
    结束日期：<input name="queryEndRegisterTimeStr" class="Wdate " id="end" autocomplete="off" readonly="readonly"
