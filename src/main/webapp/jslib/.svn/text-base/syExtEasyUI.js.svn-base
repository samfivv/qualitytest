...
$.extend($.fn.validatebox.defaults.rules, {
	eqName : {/* 验证登录名 */
		validator : function(value) {
			 var reg = /^[^_][a-zA-Z0-9_]{2,9}$/;
		       return reg.test(value);
		},
		message : '长度在3-10之间的不能以下划线开头的只包含大小写英文、数字和下划线！'
	}
});


$.extend($.fn.validatebox.defaults.rules, {
	eqDate : {/* 验证开始日期是否大于结束日期 */
		validator : function(value, param) {
			if($(param[0]).val().length>0&&value.length>0){   
			      var startDateTemp = $(param[0]).val().split(" ");   
	...
	eqDateNoTime : {/* 验证开始日期是否大于结束日期 */
		validator : function(value, param) {
			if($(param[0]).val().length>0&&value.length>0){   
			      ...
			      var arrStartDate = startDateTemp[0].split("-");   
			      var arrEndDate = endDateTemp[0].split("-");   
			      var arrStartTime = "00:00:00".split(":");  
			      var arrEndTime = "00:00:00".split(":");   
			      var allStartDate = new Date(arrStartDate[0],arrStartDate[1],arrStartDate[2],arrStartTime[0],arrStartTime[1],arrStartTime[2]);   
			      var allEndDate = new Date(arrEndDate[0],arrEndDate[1],arrEndDate[2],arrEndTime[0],arrEndTime[1],arrEndTime[2]);   
			      if(allStartDate.getTime()<allEndDate.getTime()){   
			       return true;   
...
//视频播放
sy.playDialog = function(options) {
	var opts = $.extend({
		title : '&nbsp;',
		width : 630,
		height : 500,
