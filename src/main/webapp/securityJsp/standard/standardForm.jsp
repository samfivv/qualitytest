...
<%
    ...
    String id=request.getParameter("standardId");
    if(id==null){
	id="";
}
%>
 ...
 <link rel="stylesheet" href="<%=contextPath %>/jslib/kindeditor-4.1.7/themes/default/default.css" />
		...
		<script>
			...
			KindEditor.ready(function(K) {
				editor = K.create('textarea[name="standardText"]', {
					 uploadJson : '<%=contextPath%>/mdmy/standard/plupload',
...
var submitNow = function($dialog, $grid, $pjq) {
	var url;
	 if ($(':input[name="standardId"]').val().length > 0) {
		url = '<%=contextPath%>/mdmy/standard/update';
	} else {
		url = '<%=contextPath%>/mdmy/standard/save';
	} 
	$.post(url, sy.serializeObject($('form')), function(result) {
		parent.sy.progressBar('close');//关闭上传进度条
        if(url=='<%=contextPath%>/mdmy/standard/save'){
        	if (result.success) {
    ...
    $(function() {
	if ($(':input[name="standardId"]').val().length > 0) {
		 var standardSort=$('#standardSort').val();
		 $("#selected").combobox("setValue", standardSort);
	}
	uploader = new plupload.Uploader({//上传插件定义
		...
		url : '<%=contextPath%>/mdmy/standard/upload',//上传文件路径
	...
	uploader.bind('FileUploaded', function(up, file, info) {//上传完毕
		...
		if (response.status) {
			...
			$(':input[name="standardImgUrl"]').val(response.fileUrl);
		}
	});
	uploader.init();
});
     var submitForm = function($dialog, $grid, $pjq) {
    	 var standardContent=$('#standardText').val();
  	   if(standardContent==""){
  		  parent.$.messager.alert('提示', "标准内容不能为空", 'error');
  		  return;
  		}
...
<body>
	<form method="post" class="form" >
		<input name="standardId" value="<%=id %>" type="hidden"/>
		<input id="standardSort" value="${standard.standardSort}" type="hidden"/>
		<fieldset>
			<legend>标准设置</legend>
			<table class="table" style="width: 100%;">
				    <tr>
					<th>标准标题</th>
					<td><input name="standardTitle"  class="easyui-validatebox" data-options="required:true" value="${standard.standardTitle }" /></td>
					</tr>
					<tr>
					<th>标准内容</th>
				    <td>
				   <textarea id="content" name="standardText"  style="width:580px;height:200px;visibility:hidden;">${standard.standardText }</textarea>
				    </td>
				   </tr>
					<tr>
					<th>标准排序</th>
					<td>
					<select id="selected" class="easyui-combobox" name="standardSort"  data-options="panelHeight:'200',editable:false" style="width: 155px;">
					...
					<input name="standardImgUrl" readonly="readonly" style="display: none;" /> <img id="photo" src="${standard.standardImgUrl}" style="width: 200px; height: 200px;cursor:default"></td>
					</tr>
