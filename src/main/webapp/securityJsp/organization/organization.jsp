...
<script type="text/javascript">
	...
	var addFun = function() {
		var dialog = parent.sy.modalDialog({
			...
			width:700,
			height:580,
			url : sy.contextPath + '/securityJsp/organization/organizationForm.jsp',
			buttons : [ {
				text : '添加',
				handler : function() {
	...
	var editFun = function(id) {
		var dialog = parent.sy.modalDialog({
			...
			width:700,
			height:580,
			url : sy.contextPath + '/securityJsp/organization/organizationForm.jsp?recommendUserId=' + id,
			buttons : [ {
				text : '编辑',
				handler : function() {
					dialog.find('iframe').get(0).contentWindow.submitForm(dialog, grid, parent.$);
	...
	var removeFun = function(id) {
		parent.$.messager.confirm('询问', '您确定要删除此机构吗？', function(r) {
			if (r) {
				$.post('<%=contextPath%>/mdmy/recommendUser/deleteOrganization', {
					recommendUserId : id
				}, function() {
	...
	var exportFun = function(){
		...
		f.method="post";
		f.action = sy.contextPath + '/mdmy/recommendUser/exportRecommendOrganization?'+$form.serialize()+"&sort="+recommendSort+"&order="+recommendOrder;
		f.submit();
	};
	$(function() {
		grid = $('#grid').datagrid({
			title : '',
			url : '<%=contextPath%>/mdmy/recommendUser/findAllOrganization',
			striped : true,
			rownumbers : true,
			pagination : true,
			singleSelect : true,
