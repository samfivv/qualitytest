		...
		<script>
			...
			KindEditor.ready(function(K) {
				editor = K.create('textarea[name="newsText"]', {
					 uploadJson : '<%=contextPath%>/mdmy/news/plupload',
...
var submitNow = function($dialog, $grid, $pjq) {
	var url;
	 if ($(':input[name="newsId"]').val().length > 0) {
		url = '<%=contextPath%>/mdmy/news/update';
	} else {
		url = '<%=contextPath%>/mdmy/news/save';
	} 
	$.post(url, sy.serializeObject($('form')), function(result) {
		parent.sy.progressBar('close');//关闭上传进度条
        if(url=='<%=contextPath%>/mdmy/news/save'){
        	if (result.success) {
	...
	if ($(':input[name="newsId"]').val().length > 0) {
		 ...
		 $("#selected").combobox("setValue", newsSort);
	}
	uploader = new plupload.Uploader({//上传插件定义
		...
		url : '<%=contextPath%>/mdmy/news/upload',//上传文件路径
	...
	uploader.bind('FileUploaded', function(up, file, info) {//上传完毕
		...
		if (response.status) {
			...
			$(':input[name="newsImgUrl"]').val(response.fileUrl);
		}
	});
	uploader.init();
});
     var submitForm = function($dialog, $grid, $pjq) {
    	 var newsContent=$('#newsText').val();
  	   if(newsContent==""){
  		  parent.$.messager.alert('提示', "新闻内容不能为空", 'error');
  		  return;
  		}
...
<body>
	<form method="post" class="form" >
		...
		<input id="newsSort" value="${news.newsSort}" type="hidden"/>
		<fieldset>
			<legend>新闻设置</legend>
			<table class="table" style="width: 100%;">
				    <tr>
					<th>新闻标题</th>
					<td><input name="newsTitle"  class="easyui-validatebox" data-options="required:true" value="${news.newsTitle }" /></td>
					</tr>
					<tr>
					<th>新闻内容</th>
				    <td>
				   <textarea id="content" name="newsText"  style="width:580px;height:200px;visibility:hidden;">${news.newsText }</textarea>
				    </td>
				   </tr>
					<tr>
					<th>新闻排序</th>
					<td>
					<select id="selected" class="easyui-combobox" name="newsSort"  data-options="panelHeight:'200',editable:false" style="width: 155px;">
					...
					<input name="newsImgUrl" readonly="readonly" style="display: none;" /> <img id="photo" src="${news.newsImgUrl}" style="width: 200px; height: 200px;cursor:default"></td>
					</tr>
				    <tr>
					<th>新闻来源</th>
					<td><input name="newsSourceFrom"  class="easyui-validatebox" data-options="required:false" value="${news.newsSourceFrom }" /></td>
					</tr>
